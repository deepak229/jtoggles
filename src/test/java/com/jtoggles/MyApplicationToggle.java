...
import com.jtoggles.model.ToggleConfidenceLevel;
...
public enum MyApplicationToggle
    ...
    TEST(new TenantedToggle("abc", ToggleConfidenceLevel.PRODUCTION, null, (toggle, tenant, newState) -> {
    ...
    TEST2(new ServiceToggle("Service Toggle", ToggleConfidenceLevel.INTERNAL, Collections.singleton(TEST),
        (toggle, newState) -> {
            final Logger logger = Logger.getLogger(MyApplicationToggle.class);
            logger.info(String.format("toggle %s changed to %s", toggle.getName(), newState));
        })),

    TEST3(new ServiceToggle("Service Toggle", ToggleConfidenceLevel.PRODUCTION,null,
        (toggle, newState) -> {
        final Logger logger = Logger.getLogger(MyApplicationToggle.class);
        logger.info(String.format("toggle %s changed to %s", toggle.getName(), newState));
    }));
    ...
    public static void main(final String[] args) {
        ...
        final Logger logger = Logger.getLogger(MyApplicationToggle.class);
        final ToggleManager m = new ToggleManagerImpl(ToggleConfidenceLevel.INTERNAL, MyApplicationToggle.class);
